...
import { type Author, authors, books, db, type NewBook } from "@book-manager/database";
import { and, asc, count, eq, ilike, type SQL } from "drizzle-orm";
import { revalidatePath } from "next/cache";
import type { ActionResult, Book, BookResponse, Query } from "@/model/book";
...
export async function submitBook(book: NewBook): Promise<ActionResult> {
  try {
    if (
      book.isbn !== null &&
      book.isbn &&
      (await db.select().from(books).where(eq(books.isbn, book.isbn))).length > 0
    ) {
      return { success: false, error: `ISBN: ${book.isbn} is already in use.` };
    }
    ...
    return {
      success: false,
      error: `An unexpected Error occured while trying to add Book with Title: ${book.title}`,
    };
  }
}
...
export async function deleteBook(id: number): Promise<ActionResult> {
  try {
    ...
    return {
      success: false,
      error: `An unexpected Error occured while trying to delete book with ID: ${id}`,
    };
  }
}
...
export async function saveBook(book: Book): Promise<ActionResult> {
  try {
    ...
    return {
      success: false,
      error: `An unexpected Error occured while trying to save Book with ID: ${book.id}`,
    };
  }
}
